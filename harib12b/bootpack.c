...
void HariMain(void)
{
	...
	int key_to = 0, key_shif = 0, key_leds = (binfo->leds >> 4) & 7, keycmd_wait = -1;
	...
	sheet_setbuf(sht_cons, buf_cons, 256, 165, -1); // 设置为无透明色
	...
	*((int *)(task_cons->tss.esp + 8)) = memtotal;
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			io_sti();
			if (256 <= i && i <= 511) { /* 键盘数据 */
				// sprintf(s, "%d  %02X", key_leds & 4, i - 256);
				...
				{
					if ((((key_leds & 4) == 0) && key_shif ==0)
						|| ((key_leds & 4 != 0 && key_shif != 0)))
					{
						s[0] += 0x20;// 将大写转化为小写
					}
				}
				...
				if (i == 256 + 0x0f) // TAB 键
				{
					...
					else
					{
						...
						fifo32_put(&task_cons->fifo, 3);// 命令行窗口光标ON
					}
		...
		else
		{
			...
			if (256 <= i && i <= 511) // 键盘数据（通过任务A）
			{
				...
				else if (i == 10 + 256) // 回车键
				{
					...
					else if (strcmp(cmdline, "cls") == 0) // 清屏
					{
						for(y = 28; y < 28 + 128; y++)
						{
							for(x = 8; x < 8 + 240; x ++)
					...
					else if (strcmp(cmdline, "dir") == 0) // dir 命令
					{
						for (x =0 ; x < 224; x++)
						{
							...
							if (finfo[x].name[0] != 0xe5)// 第一个为oxe5 时，为已删除文件
							{
				...
				else
				{
					...
					if (cursor_x < 240)
					{
						s[0] = i - 256;
						s[1] = 0;
						cmdline[cursor_x /8 -2] = i - 256;// 把每个输入字符保存，自己加的注释
						putfonts8_asc_sht(sheet, cursor_x, cursor_y, COL8_FFFFFF, COL8_000000, s, 1);
						cursor_x += 8;
					}
				}
